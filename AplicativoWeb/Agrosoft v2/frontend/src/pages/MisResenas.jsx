...
export default function MisResenas() {
  ...
  useEffect(() => {
    ...
    const fetchReviews = async () => {
      ...
      try {
        setLoading(true);
        const res = await axios.get(`${API_URL}/clienteresenas/user/${storedUser.id_usuario}`);
        if (mounted) {
          if (res.data.success) {
  ...
  const saveEdit = async (id) => {
    try {
      ...
      const res = await axios.put(`${API_URL}/clienteresenas/${id}`, payload);
      if (res.data.success) {
  ...
  const deleteReview = async (id) => {
    if (!window.confirm('¿Eliminar reseña? Esta acción no se puede deshacer.')) return;
    try {
      const usuario = JSON.parse(localStorage.getItem('user'));
      const res = await axios.delete(`${API_URL}/clienteresenas/${id}`, { data: { id_usuario: usuario.id_usuario } });
      if (res.data.success) {
